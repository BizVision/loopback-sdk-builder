  ...
  **/
  ...
  Object.keys(ctx.models).forEach(modelName => {
    if (ctx.models[modelName].sharedClass.ctor.settings.sdk &&
      ...
      var plural = ctx.models[modelName].sharedClass.ctor.settings.plural || ejs.filters.pluralize(modelName);
      var http = ctx.models[modelName].sharedClass.ctor.settings.http;
      var httpPath = http && http.path || plural;
      schema.push(
        /**
        * SDK MODELS
        */
      ...
      if (ctx.fireloopOnly === 'disabled' || (ctx.models[modelName].sharedClass.ctor.settings.base === 'User')) {
        schema.push({
          ...
          params: {
            ...
            buildObservableType,
            paramIsContext,
            paramIsServerInternal,
            paramIsRequestOptions
          }
        });
      }
    }
  ...
  function buildMethodParams(model, methodName, params, isIo) {
    ...
    params = params.filter(param => {
      return (!paramIsContext(param) && !paramIsServerInternal(param) && !paramIsRequestOptions(param));
    });
  ...
  /**
   * @method paramIsRequestOptions
   * @description
   * Testing if the param is a http optionsFromRequest
   */
  function paramIsRequestOptions(param) {
    return (typeof param.http !== 'undefined' && (param.http === 'optionsFromRequest' ||
      (typeof param.http === 'function' && param.http.name === 'createOptionsViaModelMethod')));
  }
  /**
   * @method paramIsServerInternal
   * @description
   * Testing if the param used internal to the server only and the client should not provide it.
   */
  function paramIsServerInternal(param) {
    return (typeof param.http !== 'undefined' && (typeof param.http === 'function' ||
      !!(param.http.source && param.http.source.match(/req|res|context/))));
  }
  /**
